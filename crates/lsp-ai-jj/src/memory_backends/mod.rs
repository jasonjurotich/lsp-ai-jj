...
impl TryFrom<Config> for Box<dyn MemoryBackend + Send + Sync> {
  ...
  fn try_from(configuration: Config) -> Result<Self, Self::Error> {
    match configuration.config.memory.clone() {
      ValidMemoryBackend::FileStore(file_store_config) => {
        info!("Initializing FileStore memory backend."); // Add logging
        Ok(Box::new(file_store::FileStore::new(
          file_store_config,
          configuration,
        )?))
      }
      ValidMemoryBackend::VectorStore(vector_store_config) => {
        info!("Initializing VectorStore memory backend.");
        Ok(Box::new(vector_store::VectorStore::new(
          vector_store_config,
          configuration,
        )?))
      }
      ValidMemoryBackend::PostgresML(postgresml_config) => {
        info!("Initializing PostgresML memory backend.");
        Ok(Box::new(postgresml::PostgresML::new(
          postgresml_config,
          configuration,
        )?))
      }
      ValidMemoryBackend::SurrealDbStore(surreal_config) => {
        info!("Initializing SurrealDbStore memory backend.");
        let store = crate::utils::TOKIO_RUNTIME.block_on(
          surrealdb_store::SurrealDbStore::new(surreal_config, configuration),
        )?;
        Ok(Box::new(store))
      }
    }
