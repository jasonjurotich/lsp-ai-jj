...
use lsp_types::{
  ...
  CompletionList, CompletionParams, CompletionResponse, Position, Range,
...
static RE: Lazy<Mutex<HashMap<String, Regex>>> =
...
fn post_process_start(response: String, front: &str) -> String {
  ...
  loop {
    if response_chars.is_empty()
      || front_chars.ends_with(&response_chars[..front_match])
    {
      break;
    } else {
...
fn do_run(
  ...
  let max_requests_per_second =
    config.get_completion_transformer_max_requests_per_second();
  ...
  match request {
    WorkerRequest::Completion(request) => {
      ...
      let transformer_backend = transformer_backends
        ...
        .with_context(|| {
          format!("can't find model: {}", &completion_config.model)
        })?;
      do_completion(transformer_backend, memory_backend_tx, &request, &config)
        .await
    }
...
async fn do_code_action_action_resolve(
  ...
  if matches!(prompt, Prompt::ContextAndCode(_))
...
async fn do_code_action_resolve(
  ...
  let action =
    if let Some(chat_action) = config.get_chats().iter().find(|chat_action| {
      chat_action.action_display_name == request.params.title
    }) {
...
async fn do_code_action_request(
  ...
  let enabled_chats =
    futures::future::join_all(chats.iter().map(|chat| async {
      let (tx, rx) = oneshot::channel();
      memory_backend_tx.clone().send(
        memory_worker::WorkerRequest::CodeActionRequest(
...
async fn do_completion(
  ...
  if let Some(post_process) = config.get_completions_post_process() {
    response.insert_text =
      post_process_response(response.insert_text, &prompt, post_process);
  }
...
async fn do_generate(
  ...
  info!(
    "Temporarily bypassing history parser, using empty history for testing."
  );
    ...
    if is_gemini {
      ...
      match serde_json::to_value(chat_history) {
        ...
        Err(e) => {
          ...
          return Err(anyhow!(
            "Failed to serialize chat history messages: {}",
            e
          ));
        }
      }
...
mod tests {
  ...
  fn test_post_process_fim() {
    ...
    let new_response =
      post_process_response(response.clone(), &prompt, &config);
