...
use crate::{
  ...
  utils::{
    chunk_to_id, format_file_chunk, tokens_to_estimated_characters,
    TOKIO_RUNTIME,
  },
};
...
use super::{
  ...
  ContextAndCodePrompt, FIMPrompt, MemoryBackend, MemoryRunParams, Prompt,
  PromptType,
};
...
async fn split_and_upsert_file(
  ...
  let chunks =
    chunks.with_context(|| format!("file not found for splitting: {uri}"))?;
...
impl PostgresML {
  ...
  pub(crate) fn new(
    ...
    let database_url = if let Some(database_url) =
      postgresml_config.database_url.clone()
    {
      database_url
    } else {
    ...
    let collection_name = match configuration.client_params.root_uri.clone() {
      Some(root_uri) => format!(
        "{:x}",
        md5::compute(
          format!("{root_uri}_{}", serde_json::to_string(&pipeline)?)
            .as_bytes()
        )
      ),
      None => {
        ...
        if !new_uris.is_empty() {
          ...
          let chunks: Vec<Vec<Chunk>> = match file_uris
            ...
            .map(|uri| {
              ...
              let file = file_store.get(uri).with_context(|| {
                format!("getting file for splitting: {uri}")
              })?;
      ...
      crawl
        ...
        .maybe_do_crawl(triggered_file, |config, path| {
          ...
          if current_bytes >= 10_000_000
            || total_bytes as u64 >= config.max_crawl_memory
          {
...
impl MemoryBackend for PostgresML {
  ...
  async fn build_prompt(
    ...
    let total_allowed_characters =
      tokens_to_estimated_characters(params.max_context);
    ...
    let code = self.file_store.build_code(
      position,
      prompt_type,
      file_store_params,
      false,
    ...
    let context = res
      ...
      .join("\n\n");
    let context =
      &context[..(total_allowed_characters - chunk_size).min(context.len())];
