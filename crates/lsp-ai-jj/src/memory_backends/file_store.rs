...
impl FileStore {
  ...
  pub(crate) fn position_to_byte(
    ...
    let file = file_map.get(&uri).with_context(|| {
      format!("trying to get file that does not exist {uri}")
    })?;
    let line_char_index = file
      .rope
      .try_line_to_char(position.position.line as usize)?;
    Ok(file.rope.try_char_to_byte(
      line_char_index + position.position.character as usize,
    )?)
  }
}
...
impl MemoryBackend for FileStore {
  ...
  fn changed_text_document(
    ...
    let file = file_map.get_mut(&uri).with_context(|| {
      format!("Trying to get file that does not exist {uri}")
    })?;
    for change in params.content_changes {
      ...
      if let Some(range) = change.range {
        ...
        let end_index = file.rope.line_to_char(range.end.line as usize)
...
impl FileStore {
  pub(crate) fn default_with_filler_file() -> anyhow::Result<Self> {
    ...
    let file_store_config =
      if let config::ValidMemoryBackend::FileStore(file_store_config) =
...
mod tests {
  ...
  fn generate_filler_text_document(
    uri: Option<&str>,
    text: Option<&str>,
  ) -> TextDocumentItem {
...
<CURSOR>"#
  ...
  fn test_file_store_tree_sitter() -> anyhow::Result<()> {
    ...
    let file_store =
      FileStore::new_with_params(file_store_config, config, params)?;
