...
fn quantize(embedding: &[f32]) -> Vec<u8> {
  ...
  let bytes: Vec<u8> =
    embedding.iter().map(|x| x.clamp(0., 1.) as u8).collect();
...
impl VectorStoreInner {
  ...
  fn search(
    ...
    let results: anyhow::Result<Vec<BTreeMap<_, _>>> = self
      .store
      .par_values()
        ...
        for chunk in chunks {
          let score = match (&chunk.vec, &scv_embedding) {
            (StoredChunkVec::F32(vec1), StoredChunkVec::F32(vec2)) => {
              #[cfg(feature = "simsimd")]
              {
                OrderedFloat(SpatialSimilarity::dot(vec1, vec2).context(
                  "vector length mismatch when taking the dot product",
                )? as f32)
              }
              #[cfg(not(feature = "simsimd"))]
              {
              ...
              #[cfg(feature = "simsimd")]
              {
                OrderedFloat(
                  embedding.len() as f32
                    - BinarySimilarity::hamming(vec1, vec2).context(
                      "vector length mismatch when taking the hamming distance",
                    )? as f32,
                )
              }
              #[cfg(not(feature = "simsimd"))]
        ...
        let sub_result_score = if rerank_top_k.is_some() {
          match &sub_result_chunk.vec {
            ...
            StoredChunkVec::F32(_) => {
              warn!(
                "Not reranking in vector_store because vectors are not binary"
              );
              sub_result_score
            }
          }
      ...
      loop {
        ...
        if !new_uris.is_empty() {
          ...
          for uri in file_uris {
            ...
            let chunks_to_upsert =
              match task_vector_store.read().store.get(&uri) {
